...
for gpu_count in "${GPU_COUNTS[@]}"; do
    echo "========================================"
    echo "Running experiment with GPU_COUNT=$gpu_count, TARGET_QPS=$TARGET_QPS"
    echo "========================================"

    # Build log directory name
    if [[ -n "$BLOCK_SIZE" ]]; then
        EXP_LOG_DIR="${BASE_LOG_DIR}/exp__${LOG_PREFIX}tp_${gpu_count}_pp_${PIPELINE_PARALLEL_SIZE}_${BLOCK_SIZE}_${MAX_MODEL_LEN}_${MAX_NUM_BATCHED_TOKENS}_${GPU_MEMORY_UTILIZATION}_qps${TARGET_QPS}"
    else
        EXP_LOG_DIR="${BASE_LOG_DIR}/exp__${LOG_PREFIX}tp_${gpu_count}_pp_${PIPELINE_PARALLEL_SIZE}_${MAX_MODEL_LEN}_${MAX_NUM_BATCHED_TOKENS}_${GPU_MEMORY_UTILIZATION}_qps${TARGET_QPS}"
    fi
    mkdir -p "${EXP_LOG_DIR}"
    ...
    # Run the experiment with error handling
    {
        ...
        if [ ${PIPESTATUS[0]} -eq 0 ]; then
            echo "Experiment with GPU_COUNT=$gpu_count SUCCEEDED" | tee -a "$EXECUTION_LOG"
        else
